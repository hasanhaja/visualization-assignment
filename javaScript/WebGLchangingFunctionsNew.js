...
function readSEbox(mode) {
    ...
    if (n1 && n2 || n1 == 0 || n2 == 0) {
        if (n1 <= 0 || n2 <= 0 || n1 > 5 || n2 > 5) {
            ...
            if (mode == 0)  { }// to do call changeShape appropriately 
        }
    }
    else {
...
function userImageDefault(num) {
    if (num == 2) {
        contextUI.save();
        contextUI.drawImage(Images.UserDefault, 0, 0, canvasUI.width, canvasUI.height);
    }
    else {
        contextUC.save();
        contextUC.drawImage(Images.UserDefault, 0, 0, canvasUC.width, canvasUC.height);
    }
}
...
function changeColour2(n, mode) {
    if (n == 3) {
        ...
        if (State.colour != 3) {
            ...
            if (canvasUC.style.opacity == 1) { //Texture mapping reopened after using it before (so there is an image)
                State.colour = 3;
        ...
        if (mode != 1 && document.getElementById("textureColour").style.display == "block") {
...
function pickColor(mode) {
    ...
    var r = parseInt(((x.charAt(0) == "#") ? x.substring(1, 7) : x).substring(0, 2), 16);
    var g = parseInt(((x.charAt(0) == "#") ? x.substring(1, 7) : x).substring(2, 4), 16);
    var b = parseInt(((x.charAt(0) == "#") ? x.substring(1, 7) : x).substring(4, 6), 16);
...
function changeUserImage(id, cl, rot, mode) {
    ...
    if (!selected_file || !selected_file.type.match('image.*')) {
        ...
        if (State.image == 9) {//Deactivate previous one (it was being used)
    ...
    img.onload = function () {
        ...
        if (State.image == 9 && mode == 0) {//It was activated, so use the new one
...
function processImage(pix) {
    ...
    for (let j = 0; j < State.N - 2; j++) {
        ...
        pix[ind + 1] = (pix[ind + 1] + pix[ind + 4 * (State.N) * (State.N - 1) + 1]) / 2;
        pix[ind + 2] = (pix[ind + 2] + pix[ind + 4 * (State.N) * (State.N - 1) + 2]) / 2;
    ...
    pix[0] = (pix[0] + pix[4 * (State.N - 1)] + pix[4 * (State.N - 1) * (State.N)] + pix[4 * (State.N) * (State.N) - 4]) / 4;
    pix[1] = (pix[1] + pix[4 * (State.N - 1) + 1] + pix[4 * (State.N - 1) * (State.N) + 1] + pix[4 * (State.N) * (State.N) - 4 + 1]) / 4;
    pix[2] = (pix[2] + pix[4 * (State.N - 1) + 2] + pix[4 * (State.N - 1) * (State.N) + 2] + pix[4 * (State.N) * (State.N) - 4 + 2]) / 4;
...
function computeHValues(pix) {
    ...
    for (let i = 0; i < State.N - 1; i++) {
        for (let j = 0; j < State.N - 1; j++) {
            wGL.VertexHValues[count] = ((pix[index] + pix[index + 1] + pix[index + 2]) / 255) / 3;
            wGL.VertexHValues[count + 1] = ((pix[index + 4 * State.N] + pix[index + 4 * State.N + 1] + pix[index + 4 * State.N + 2]) / 255) / 3;
            wGL.VertexHValues[count + 2] = ((pix[index + 4 * State.N + 4] + pix[index + 4 * State.N + 5] + pix[index + 4 * State.N + 6]) / 255) / 3;
            wGL.VertexHValues[count + 3] = ((pix[index + 4 * State.N + 4] + pix[index + 4 * State.N + 5] + pix[index + 4 * State.N + 6]) / 255) / 3;
            wGL.VertexHValues[count + 4] = ((pix[index + 4] + pix[index + 5] + pix[index + 6]) / 255) / 3;
            wGL.VertexHValues[count + 5] = ((pix[index] + pix[index + 1] + pix[index + 2]) / 255) / 3;
            count += 6;
            index += 4;
        }
        index += 4;
...
function convert(pix) {
    ...
    for (let i = 0; i < State.N; i++)
        mat[i] = new Array(State.N);

    var count = 0;
    for (let i = 0; i < State.N; i++)
        for (let j = 0; j < State.N; j++) {
            mat[i][j] = 1.0-(0.33 * pix[count] + 0.33 * pix[count + 1] + 0.33 * pix[count + 2]) / 255;
            count += 4;
        }
    return mat;
}
...
function bumpMap() {
    ...
    if (checkBox.checked == true) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, wGL.VertexColourBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(VertexColourData), gl.STATIC_DRAW);
...
function DrawAndExtract(id, im, Nctx) {
    ...
    if (Nctx == 3) {
        ...
        if (auxMax < 1.0 && auxMax > 0.0) State.dispMax = 1.0 - auxMax; else State.dispMax = 1.0;
    }
 }
...
* Auxiliary function to rotate the customised images in the 2d canvas elements
    ...
    function rotateImage(num, dir) {
        var ctx;
        if (num == 2) ctx = contextUI; else ctx = contextUC;
        ...
        gl.uniform1i(wGL.bumpMap, 0);
        if (num == 2) changeUserImage("imageCanvas", "imD", 1, 0); else changeUserImageColour2(1, 0);
    }
...
function changeImage2(num, id, cl, mode) {
    if (num != State.image || mode == 1) { //Different image from the actual one
        ...
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(wGL.VertexHValues), gl.STATIC_DRAW);
        gl.enableVertexAttribArray(wGL.vertexHValueAttribute);
        gl.vertexAttribPointer(wGL.vertexHValueAttribute, wGL.VHBufferItemSize, gl.FLOAT, false, 0, 0);
        //Image 9 is the customised one and its information has been already extracted
        if (num == 9) gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(wGL.VertexHValues), gl.STATIC_DRAW);
